...
export default defineConfig(({ mode }) => {
  ...
  const darwinProxyTarget = env.VITE_DARWIN_API_ORIGIN || process.env.VITE_DARWIN_API_ORIGIN || 'https://api-darwin.railstatistics.co.uk'
  const darwinApiKey = env.DARWIN_API_KEY || process.env.DARWIN_API_KEY || ''
  return {
  test: {
    environment: 'jsdom',
    globals: true,
  ...
  server: {
    ...
    proxy: {
      '/api/darwin': {
        ...
        changeOrigin: true,
        headers: darwinApiKey ? { 'X-API-Key': darwinApiKey } : undefined,
